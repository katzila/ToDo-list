...
function App() {// основной функциональный компонент приложения
    ...
    const changeTaskContent=(id,newlabel,newtext)=>{ // обновление заголовка и содержания задачи на пришедные из компонента content
        setTasks([
            ...tasks.map((task) => 
    ...
    const changeActiveTo = (taskToChange) => { //меняем активную задачу на пришедшую
        ...
        setTasks([
            ...tasks.map((task) => 
                task.id === taskToChange.id ? { ...task, isActive: true } : { ...task, isActive: false }  // в копии массива ищем задачу с id как у нашей и активируем, а остальные дизактивируем
            )
        ])
...
export default App; //экпортируем компонент для дальнейшего пользования